...
void __PrintOnLcd(Lcd_HandleTypeDef *lcd) {

static  uint8_t display_counter = 0;
	display_counter =
			(display_counter + 1 > DISPLAY_COUNTER) ? 0 : display_counter;

	switch (display_counter) {
	case FIRST_VARIABLE: {
		Lcd_clear(lcd);
		Lcd_cursor(lcd, 0, 1);
		Lcd_string(lcd, "FIRST_VARIABLE");
		break;
	}
	case SECOND_VARIABLE: {
		Lcd_clear(lcd);
		Lcd_cursor(lcd, 0, 1);
		Lcd_string(lcd, "SECOND_VARIABLE");
		break;
	}
	case THIRD_VARIABLE: {
		Lcd_clear(lcd);
		Lcd_cursor(lcd, 0, 1);
		Lcd_string(lcd, "THIRD_VARIABLE");
		break;
	}
	case FOURTH_VARIABLE: {
		Lcd_clear(lcd);
		Lcd_cursor(lcd, 0, 1);
		Lcd_string(lcd, "FOURTH_VARIABLE");
		break;
	}
	case FIFTH_VARIABLE: {
		Lcd_clear(lcd);
		Lcd_cursor(lcd, 0, 1);
		Lcd_string(lcd, "FIFHT_VARIABLE");
		break;
	}
	}
